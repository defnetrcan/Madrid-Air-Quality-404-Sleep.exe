...
`PM10`, `PM2.5`, `BENCENO`, `CO`, `ETILBENCENO`, `HIDROCARBSNOMETANICOS`, `HIDROCARBS_TOTALES`, `METANO`, `NO`, `NO2`, `NOX`, `O3`, `SO2`, `TOLUENO`
...
`TI` = traffic intensity, `SP` = average speed, `OC_` = road occupancy/congestion.
Suffixes: `IDW`, `KRIGING`, `RBFGAUSSIAN`, `RBFLINEAR`, `RBF_MULTIQUADRIQ`.
...
Accepts either a directory of yearly CSVs (`metraqaq-2001.csv` … `metraqaq-2024.csv`) or a single CSV (sample mode). Auto-creates `figures/`, `corr_matrices/`, and `partitions/`.
...
<table style="border-collapse:collapse;width:100%;background:#ffffff;margin:10px 0;">
 ...
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">`CORR_THR`</td>
  ...
  <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">Meaningful association threshold; r² = 0.36, so connected pairs share about 36% variance.</td>
 </tr>
 <tr>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">`TASK3EVALMAX_SENSORS`</td>
 ...
 <tr>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">`TASK3EVALN_SEEDS`</td>
 ...
 <tr>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">`TASK3EVALMASK_FRAC`</td>
 ...
 <tr>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">`TASK8MAXWORKERS`</td>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">`2`</td>
 ...
 <tr>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">`TASK8CORRMIN_PERIODS`</td>
...
Time coverage: 2001-01-01 00:00:00 to 2024-12-31 23:00:00
```

Shared window for selected pollutants [NO2, O3, PM10, SO2]: 2001-01-01 to 2024-12-31. All four pollutants span the full period, so no window restriction is needed for cross-pollutant analyses.
...
### Figure: task1no2monthly.png

![NO2 Monthly Trend and Sensor Count](figures/task1no2monthly.png)
...
### Figure: task1airquality_distributions.png

![Air Quality Variable Distributions](figures/task1airquality_distributions.png)
...
### Figure: task1meteorologydistributions.png

![Meteorology Distributions](figures/task1meteorologydistributions.png)
...
### Figure: task1trafficdistributions.png

![Traffic Variable Distributions](figures/task1trafficdistributions.png)
...
METRAQ has filled every missing value before release, there are zero NaN values in the raw data. The `isinterpolated == True` flag marks positions that were originally absent and filled by the dataset authors. We reverse-engineer original missingness from this flag, stripping `isinterpolated == True` entries before applying our own imputation. This is the correct methodology: we treat METRAQ's interpolation as the "official" fill that we will compare against.
...
```
 magnitudename missingrate
 PRE 87.68% from meteorology only from 2019
 RS 87.61%
 DV 83.63%
 VV 83.58%
 PRECIPITACION 79.29%
 ...
 PM10 2.86%
 NO2 1.76%
 TI_KRIGING 0.00% from traffic: always zero by construction
```
...
```
 sensorname magnitude maxgaph ngaps totalmissingh
...
```
Physically impossible zeros (PRE = 0 hPa): 414 from atmospheric pressure cannot be zero
Negative values where impossible: 684
 to HR: 504 records (humidity cannot be negative)
 to RS: 180 records (solar radiation cannot be negative in non-calibration hours)

Bounded-range violations:
 Invalid HR (outside 0-100%): 771
 Invalid DV (outside 0-360°): 0 from wind direction is clean
 Invalid PREC (< 0 mm): 0 from precipitation is clean

Duplicate rows on (sensorid, magnitudeid, entry_date): 0
...
<table style="border-collapse:collapse;width:100%;background:#ffffff;margin:10px 0;">
 ...
 <tr>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">OCRBFMULTIQUADRIQ</td>
 ...
 <tr>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">OCRBFGAUSSIAN</td>
...
### Figure: task2missingnesssensor_var.png

![Missingness by Sensor × Variable](figures/task2missingnesssensor_var.png)
...
### Figure: task2missingnesstemporal.png

![Temporal Missingness Heatmap](figures/task2missingnesstemporal.png)
...
ffill/bfill 24h uses backward-fill to looks into the future to excluded from final selection (kept only as a theoretical ceiling benchmark).
KNN spatial IDW and rolling24hpast are the only admissible methods for the final imputed dataset.
...
magnitude_name method rmse mae
 PM10 knn 12.075 7.500 from best causal
 PM10 ffillbfill24h 12.226 6.928 (non-causal baseline)
 PM10 rolling24hpast 16.190 10.167
 NO2 ffillbfill24h 13.790 8.841 (non-causal)
 NO2 knn 16.585 11.306 from best causal
 NO2 rolling24hpast 24.292 17.583
 O3 ffillbfill24h 11.109 7.424 (non-causal)
 O3 knn 12.664 8.990 from best causal
 O3 rolling24hpast 24.196 18.597
 SO2 ffillbfill24h 2.573 0.999 (non-causal)
 SO2 rolling24hpast 4.236 1.999 from best causal
 SO2 knn 5.521 3.343
```
...
Why rolling24hpast wins for SO2:
...
```
Pollutant Method nmissing nfilled pct_filled
NO2 METRAQ 107,881 72,817 67.5%
NO2 ffill_bfill 107,881 29,264 27.1%
NO2 knn 107,881 107,880 100.0% from always fills
...
Why ffill/bfill fills only 18-34%: It requires an observed value within 24 hours on either side. For Villaverde's 3.57-year outage, no observation exists within 24 hours to method fails entirely for that gap.
...
NO2 knn 0.0679 from most similar to METRAQ
NO2 rolling24hpast 0.3578 from most different
O3 knn 0.1937
O3 rolling24hpast 0.2694
PM10 knn 0.0706 from most similar
PM10 rolling24hpast 0.3336
SO2 rolling24hpast 0.3697 from wins on RMSE but very different distribution
```

Rolling24hpast consistently shows the highest KS distance: it smooths out the high-frequency variation, producing narrower distributions (lower variance, higher median) than the true missing-value distributions. KNN better preserves the shape because it borrows from neighbors experiencing the same concurrent conditions.
...
### Figure: task3distributionNO2.png

![NO2 Imputation Distribution](figures/task3distributionNO2.png)
...
### Figure: task3distributionO3.png

![O3 Imputation Distribution](figures/task3distributionO3.png)
...
### Figure: task3distributionPM10.png

![PM10 Imputation Distribution](figures/task3distributionPM10.png)
...
### Figure: task3distributionSO2.png

![SO2 Imputation Distribution](figures/task3distributionSO2.png)

Key observations: For SO2, rolling24hpast wins on RMSE but its distribution (narrow peak) is very different from METRAQ's broader distribution. This highlights a limitation: minimizing RMSE at pseudo-gap test points does not guarantee the imputed distribution is realistic, especially for localized pollutants.

### Figure: task3rmsecomparison.png

![Imputation RMSE Comparison](figures/task3rmsecomparison.png)

What this shows: Grouped bar chart of pseudo-gap RMSE by pollutant and causal method (KNN vs. rolling24hpast).
...
### Figure: task4realvsimputedall.png

![Real vs Imputed Monthly Mean](figures/task4realvsimputedall.png)
...
### Figure: task4yearlyNO2.png

![NO2 Yearly Trend](figures/task4yearlyNO2.png)

What this shows: Annual city-wide NO2 average (stable sensors, station-normalized) 2001-2024.

Key observations: Strong declining trend from ~54 µg/m³ (2001) to ~30 µg/m³ (2020-2024), a ~44% reduction. This reflects: progressive Euro emission standard tightening (Euro 3to6d), electrification of Madrid's bus/taxi fleet, introduction of the Madrid Central Low Emission Zone (2018). COVID-19 lockdown dip in 2020, partial recovery in 2021-2022. The 2024 value is close to the EU annual limit of 40 µg/m³, with some sensors likely exceeding it.

### Figure: task4yearlyO3.png

![O3 Yearly Trend](figures/task4yearlyO3.png)

Key observations: A slight upward trend in O3, roughly inverse to NO2's decline. This is the ozone penalty: O3 is partly suppressed by NO via the titration reaction O3 + NO to NO2 + O2. As urban NO decreases (cleaner vehicles), less O3 is consumed, background ozone accumulates. This well-documented phenomenon creates a trade-off between NO2 and O3 management in European cities.

### Figure: task4yearlyPM10.png

![PM10 Yearly Trend](figures/task4yearlyPM10.png)

### Figure: task4yearlySO2.png

![SO2 Yearly Trend](figures/task4yearlySO2.png)

Both PM10 and SO2 decline substantially. SO2 shows the steepest drop (~12 µg/m³ to ~4 µg/m³), reflecting EU sulfur cap regulations that mandated low-sulfur fuel and phased out sulfurous heating oil.

### Figure: task4monthlyNO2.png

![NO2 Monthly Seasonality](figures/task4monthlyNO2.png)

What this shows: Average NO2 by calendar month.

Key observations: Winter maximum (December-January, ~50 µg/m³), summer minimum (July-August, ~25 µg/m³). Driven by: (1) reduced solar radiation in winter to slower photolytic NO2 destruction; (2) temperature inversions trapping pollutants; (3) cold-start engine emissions.

### Figure: task4monthlyO3.png

![O3 Monthly Seasonality](figures/task4monthlyO3.png)

Key observations: Opposite to NO2, minimum in winter (~20 µg/m³), maximum in June-July (~75 µg/m³). More sunlight to more photolysis to more O3 formation. Lower NO in summer also means less O3 titration.

### Figure: task4monthlyPM10.png

![PM10 Monthly Seasonality](figures/task4monthlyPM10.png)

### Figure: task4monthlySO2.png

![SO2 Monthly Seasonality](figures/task4monthlySO2.png)
...
### Figure: task4hourlyNO2.png

![NO2 Diurnal Profile](figures/task4hourlyNO2.png)
...
### Figure: task4hourlyO3.png

![O3 Diurnal Profile](figures/task4hourlyO3.png)
...
### Figure: task4hourlyPM10.png

![PM10 Diurnal Profile](figures/task4hourlyPM10.png)

### Figure: task4hourlySO2.png

![SO2 Diurnal Profile](figures/task4hourlySO2.png)
...
### Figure: task4heatmapNO2.png

![NO2 Year × Month Heatmap](figures/task4heatmapNO2.png)
...
### Figure: task4heatmapO3.png

![O3 Year × Month Heatmap](figures/task4heatmapO3.png)

### Figure: task4heatmapPM10.png

![PM10 Year × Month Heatmap](figures/task4heatmapPM10.png)

### Figure: task4heatmapSO2.png

![SO2 Year × Month Heatmap](figures/task4heatmapSO2.png)
...
### Figure: task4sensortrends_NO2.png

![NO2 Per-Sensor Yearly Trends](figures/task4sensortrends_NO2.png)
...
### Figure: task4sensortrends_O3.png

![O3 Per-Sensor Yearly Trends](figures/task4sensortrends_O3.png)

### Figure: task4sensortrends_PM10.png

![PM10 Per-Sensor Yearly Trends](figures/task4sensortrends_PM10.png)

### Figure: task4sensortrends_SO2.png

![SO2 Per-Sensor Yearly Trends](figures/task4sensortrends_SO2.png)
...
Naive fully-connected graph:
 ...
 Implication: density=1 to no hub/periphery, community detection meaningless.
```
...
### Figure: task5degreedistribution.png

![Spatial Network Degree Distribution](figures/task5degreedistribution.png)
...
### Figure: task5spatialnetwork.png

![Spatial Network with Communities](figures/task5spatialnetwork.png)
...
### Figure: task5sensorlocations_communities.png

![Sensor Locations with Communities](figures/task5sensorlocations_communities.png)
...
Edges are based on absolute Pearson correlation, so the graph captures both same-direction and inverse relationships. Community detection is interpreted as grouping by association strength, not necessarily same-direction co-movement.
### Terminal Output, NO2 Network (Fully Connected)

```
[NO2] Threshold sweep:
 |r| >= 0.50: 276 edges, density=1.000
 |r| >= 0.60: 276 edges, density=1.000 from fully connected at our threshold
...
```
[SO2] Threshold sweep:
 |r| >= 0.60: 25 edges, density=0.556, components=2 from not fully connected
...
Later period produces denser networks: (1) more sensors active simultaneously to longer overlap to lower correlation estimation noise; (2) improved city-wide air quality management may have genuinely increased spatial homogeneity of NO2.

### Figure: task6corrmatrixNO2.png

![NO2 Correlation Matrix](figures/task6corrmatrixNO2.png)
...
### Figure: task6corrmatrixNO2realonly.png

![NO2 Raw-Only Correlation Matrix](figures/task6corrmatrixNO2realonly.png)
...
### Figure: task6corrmatrixO3.png

![O3 Correlation Matrix](figures/task6corrmatrixO3.png)
...
### Figure: task6corrmatrixPM10.png

![PM10 Correlation Matrix](figures/task6corrmatrixPM10.png)
...
### Figure: task6corrmatrixSO2.png

![SO2 Correlation Matrix](figures/task6corrmatrixSO2.png)
...
### Figure: task6corrnetNO2.png

![NO2 Correlation Network](figures/task6corrnetNO2.png)
...
### Figure: task6corrnetNO2realonly.png

![NO2 Raw-Only Network](figures/task6corrnetNO2realonly.png)

Identical to the imputed NO2 network. Confirms robustness.

### Figure: task6corrnetO3.png

![O3 Correlation Network](figures/task6corrnetO3.png)

### Figure: task6corrnetPM10.png

![PM10 Correlation Network](figures/task6corrnetPM10.png)

### Figure: task6corrnetSO2.png

![SO2 Correlation Network](figures/task6corrnetSO2.png)
...
### Figure: task6timewindowcomparison.png

![Time Window Comparison](figures/task6timewindowcomparison.png)
...
Pollutant transport in the atmosphere without wind is governed by Fick's diffusion law and its continuous-space equivalent, the heat equation: ∂C/∂t = D·∇²C. On a graph with N nodes, the spatial Laplacian ∇² is discretized to the graph Laplacian matrix L = Ddeg − A (degree matrix minus adjacency). The random-walk Laplacian Lrw = I − W (where W is row-normalized inverse-distance weight matrix) gives the discrete heat equation:
...
Laplacian Diffusion 1-step (α=0.113) 10.009 6.317 0.799 from best physical
Laplacian Diffusion Recursive (α=0.113) 23.454 15.881 -0.106 from error compounding
 Unconstrained SAR Ridge (1-step) 9.980 6.704 0.800

Diffusion-1step vs Persistence RMSE improvement: +1.56%
to Neighbour information helps at the 1-hour horizon.
...
SAR Ridge unconstrained coefficients: βself = +27.80, βneighbor = +1.03, sum = +28.84. The unconstrained model is not physically interpretable (predictions would grow without bound over time) but fits slightly better at the 1-step horizon by acting as an amplified persistence model.
...
### Figure: task7alphatuning.png

![Alpha Tuning Curve](figures/task7alphatuning.png)
...
### Figure: task7propagationmodel.png

![Diffusion Model vs Observed NO2](figures/task7propagationmodel.png)
...
### Figure: task7maemap.png

![Per-Sensor MAE Map](figures/task7maemap.png)
...
<table style="border-collapse:collapse;width:100%;background:#ffffff;margin:10px 0;">
 <thead><tr>
 <th style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;font-weight:bold;text-align:left;">Sensor</th>
 <th style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;font-weight:bold;text-align:left;">avgmeancorr</th>
...
```
[NO2] Strongest positive (traffic to more NO2):
...
[NO2] Strongest negative (meteorology to less NO2):
 VV (wind speed): r = -0.390, 100% negative from strongest consistent effect
 TEMP: r = -0.258, 100% negative
 RS (solar rad.): r = -0.234, 100% negative

[O3] Strongest positive (meteorology to more O3):
 TEMP: r = +0.633, 100% positive from classic photochemical signature
 RS: r = +0.520, 100% positive

[O3] Strongest negative:
 HR (humidity): r = -0.642, 100% negative from hot/dry conditions to max O3
```

Physical interpretation of key associations:
Wind speed to NO2 (r = −0.39, 100%): Stronger winds dilute and transport NO2 away. The most consistent negative association in the entire dataset, holds for every sensor in every year.
Congestion to NO2 (r = +0.22, 100%): Stationary vehicles in congested traffic produce more combustion emissions per km than free-flowing traffic. Consistent with stop-and-go driving increasing per-vehicle NOx output.
Temperature to O3 (r = +0.63, 100%) + Humidity to O3 (r = −0.64, 100%): The classical photochemical O3 signature. Hot, dry conditions maximize UV irradiance and ozone formation rates.
...
### Figure: task8pollutionassociations.png

![Pollution Variable Associations Heatmap](figures/task8pollutionassociations.png)
...
Standardized inputs to coefficient magnitudes directly comparable across features
Interpretable: coefficient sign = direction of effect, magnitude = relative importance
L2 penalty prevents overfitting when correlated lag features compete (e.g., NO2lag1h and NO2lag3h are highly correlated)
...
```
Effective issue window: 2019-01-02 to 2024-01-01
...
Persistence_24h 16.617 11.286 0.288
 ...
 RandomForest 14.526 10.696 0.456 from best
```
...
<table style="border-collapse:collapse;width:100%;background:#ffffff;margin:10px 0;">
 ...
 <tbody>
 <tr>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">NO2lag1h</td>
 ...
 <tr>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">TEMPlag3h</td>
 ...
 <tr>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">TEMPlag1h</td>
 ...
 <tr>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">TIIDWlag_1h</td>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">+5.88</td>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">Recent traffic to higher tomorrow's NO2</td>
 </tr>
 <tr>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">RSlag3h</td>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">−4.21</td>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">Recent solar radiation to photolysis, lower NO2</td>
 </tr>
 <tr>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">NO2lag24h</td>
...
<table style="border-collapse:collapse;width:100%;background:#ffffff;margin:10px 0;">
 ...
 <tr>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">PRElag1h</td>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">0.045</td>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">Anticyclonic high pressure to trapping</td>
 </tr>
 <tr>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">RSlag6h</td>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">0.037</td>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">Solar radiation to photolysis</td>
 </tr>
 <tr>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">NO2lag24h</td>
 ...
 <tr>
 <td style="background:#ffffff;padding:6px 14px;border:1px solid #cccccc;text-align:left;">targetweekdaysin</td>
...
The Random Forest assigns 53% of importance to `NO2lag1h`, recent NO2 concentration is by far the best predictor of future NO2. Atmospheric pressure is second-most important (anticyclones suppress vertical mixing). Calendar features capture the weekly traffic cycle.
...
### Figure: task924hforecast.png

![24h NO2 Forecast](figures/task924hforecast.png)
...
### Figure: task10panelA_temporal.png, Temporal Patterns

![Panel A Temporal](figures/task10panelA_temporal.png)
...
### Figure: task10panelB_networks.png, Network Views

![Panel B Networks](figures/task10panelB_networks.png)
...
### Figure: task10panelCdiurnalquality.png, Diurnal Profiles & Data Quality

![Panel C Diurnal Quality](figures/task10panelCdiurnalquality.png)
